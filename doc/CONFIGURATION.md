...
| bridge_cache_max_age | 0 | String | No | The number of seconds to keep in cache a Prometheus (metrics) response. If set to zero, the caching will be disabled. Can be a string with a suffix, like `2m` to indicate 2 minutes |
| bridge_cache_max_size | 1M | String | No | The maximum amount of data to keep in cache when serving bridge responses. Changes require restart. This parameter determines the size of memory allocated for the cache even if `bridge_cache_max_age` or `bridge` are disabled. Its value, however, is taken into account only if `bridge_cache_max_age` is set to a non-zero value. Supports suffixes: 'B' (bytes), the default if omitted, 'K' or 'KB' (kilobytes), 'M' or 'MB' (megabytes), 'G' or 'GB' (gigabytes).|
