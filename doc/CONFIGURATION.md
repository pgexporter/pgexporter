...
| type | postgresql | String | No | All | The server type: `postgresql` for PostgreSQL database servers, `prometheus` for Prometheus-compatible endpoints. Only used by bridge functionality. |
| user | | String | Conditional | PostgreSQL | The user name. Required for `postgresql` type, ignored for `prometheus` type |
...
- **`postgresql`** (default): PostgreSQL database server for direct metrics collection. Requires `user` property and establishes database connections. When used with bridge functionality, adds `endpoint="host:port"` labels to metrics.
- **`prometheus`**: Prometheus-compatible HTTP endpoint for metrics scraping. No `user` property required. Only functional with bridge enabled. Metrics pass through unchanged without endpoint labels.
